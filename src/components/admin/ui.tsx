  ...
  const body = (
    <>
      <div className="flex items-start justify-between gap-2">
        {Icon ? <span className={cn("flex h-6 w-6 items-center justify-center rounded-full", tone === "default" || tone === "muted" ? "bg-white/[0.05] text-muted-foreground" : tone === "good" ? "bg-success/[0.12] text-success" : tone === "warn" ? "bg-amber/[0.12] text-amber" : "bg-danger/[0.12] text-danger")}><Icon size={12} /></span> : <span />}
        {(to || onClick) && <ArrowRight size={13} className="text-faint-foreground" />}
      </div>
      <p className={cn("text-[10.5px] leading-4 text-faint-foreground", Icon && "mt-2")}>{label}</p>
      <p className={cn("mt-0.5 text-[17px] font-semibold leading-tight tabular-nums", toneText[tone])}>{loading ? "—" : value}</p>
      {note && <p className="mt-0.5 text-[11px] leading-4 text-muted-foreground">{note}</p>}
    </>
  );
  const cls = "block min-w-0 rounded-xl border border-white/[0.07] bg-white/[0.025] p-2.5 text-left";
...
export function StatGrid({ children, cols = 2 }: { children: ReactNode; cols?: 2 | 3 | 4 }) {
  return <div className={cn("grid gap-2", cols === 2 ? "grid-cols-2" : cols === 3 ? "grid-cols-2 sm:grid-cols-3" : "grid-cols-2 lg:grid-cols-4")}>{children}</div>;
}
...
export function Panel({ title, icon: Icon, action, children, className, note }: { title: string; icon?: LucideIcon; action?: ReactNode; children: ReactNode; className?: string; note?: string }) {
  return (
    <section className={cn("rounded-xl border border-white/[0.07] bg-white/[0.02] p-3", className)}>
      <header className="mb-2 flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">
          {Icon && <Icon size={14} className="shrink-0 text-primary-glow" />}
          <h2 className="truncate text-[13.5px] font-semibold text-foreground">{title}</h2>
          {note && <span className="hidden text-[10.5px] text-faint-foreground sm:inline">{note}</span>}
        </div>
        {action}
      </header>
      {children}
...
export function Rows({ children, empty }: { children: ReactNode[]; empty: string }) {
  if (!children.length) return <p className="py-5 text-center text-[12px] text-faint-foreground">{empty}</p>;
  ...
  const inner = (
    <>
      <div className="min-w-0 flex-1">
        <p className="truncate text-[12.5px] text-foreground">{primary}</p>
        {secondary && <p className="truncate text-[11px] text-faint-foreground">{secondary}</p>}
      </div>
      {(right || rightNote) && (
        <div className="shrink-0 text-right">
          {right && <p className={cn("text-[12.5px] font-semibold tabular-nums", toneText[tone])}>{right}</p>}
          {rightNote && <p className="text-[10.5px] text-faint-foreground">{rightNote}</p>}
        </div>
      )}
    </>
  );
  return <li>{onClick ? <button type="button" onClick={onClick} className="flex w-full items-center gap-2.5 py-2 text-left hover:bg-white/[0.02]">{inner}</button> : <div className="flex items-center gap-2.5 py-2">{inner}</div>}</li>;
}
...
export function Segmented<T extends string>({ value, onChange, options }: { value: T; onChange: (v: T) => void; options: { value: T; label: string }[] }) {
  return (
    <div className="flex w-full gap-0.5 overflow-x-auto rounded-full border border-white/[0.07] bg-white/[0.02] p-0.5" role="tablist">
      {options.map((o) => (
        <button key={o.value} role="tab" aria-selected={value === o.value} type="button" onClick={() => onChange(o.value)} className={cn("flex-1 whitespace-nowrap rounded-full px-2.5 py-1 text-[11.5px] font-medium", value === o.value ? "bg-white/[0.08] text-foreground" : "text-muted-foreground hover:text-foreground")}>{o.label}</button>
      ))}
    </div>
  );
}
...
export function Pill({ children, tone = "muted" }: { children: ReactNode; tone?: Tone }) {
  ...
  return <span className={cn("inline-flex items-center rounded-full px-1.5 py-0.5 text-[10.5px] font-medium", cls)}>{children}</span>;
}

/** Small inline action link */
export function InlineAction({ children, onClick, to }: { children: ReactNode; onClick?: () => void; to?: string }) {
  const cls = "inline-flex items-center gap-1 text-[11.5px] font-semibold text-primary-glow hover:underline";
...
export const inputCls = "h-9 w-full rounded-lg border border-white/[0.1] bg-white/[0.03] px-2.5 text-[13px] text-foreground placeholder:text-faint-foreground focus:border-primary/50 focus:outline-none";
