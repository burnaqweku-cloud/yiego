    ...
    <div className="flex h-full min-h-0 flex-col text-[12px]">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-3 pt-3">
        ...
        <label className="relative block px-2">
          <Search size={13} className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-faint-foreground" />
          <input
            value={query}
        ...
        {query.trim() ? (
          <ul className="mt-2 px-2" aria-label="Search results">
            ...
            {results.map((page, i) => (
              <li key={page.id}>
                <button type="button" onClick={() => go(page)} className={cn("flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left", i === 0 ? "bg-white/[0.05] text-foreground" : "text-muted-foreground hover:bg-white/[0.035] hover:text-foreground")}>
              ...
              {ADMIN_GROUPS.map((group) => {
                ...
                return (
                  <div key={group.id} className="my-0.5">
                    ...
                    {open && (
                      <ul className="relative ml-[1.35rem] mt-0.5 border-l border-white/[0.09] pb-1 pl-1.5">
