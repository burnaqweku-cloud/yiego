    ...
    <section className="mk-wrap py-10 sm:py-14">
      <div className="relative overflow-hidden rounded-3xl border border-primary/25 bg-gradient-to-br from-primary/15 via-primary/5 to-transparent p-6 sm:p-10">
        <div className="max-w-xl">
          ...
          <h2 className="mt-2 font-display text-[26px] font-semibold leading-tight text-foreground sm:text-[34px]">{state === "agent" ? "Manage your store, prices and earnings." : "Get data cheaper. Sell it your way."}</h2>
          <p className="mt-3 text-[14.5px] leading-6 text-muted-foreground">{state === "agent" ? "Your dashboard has today's sales, your earnings balance, your share link and payouts." : "Agents buy every bundle below the public price — MTN 1GB at 4.00, 10GB at 40.00 — whether it's for themselves or to sell from their own store. Nothing to prepay, from 3.00 a month."}</p>
          {state === "offer" && (
            <ul className="mt-5 grid gap-2 sm:grid-cols-3">
              {[[BadgeCheck, "Agent prices, every bundle"], [Store, "Your own store link"], [Wallet, "Earnings to your MoMo"]].map(([Icon, t]) => { const I = Icon as typeof Store; return <li key={String(t)} className="flex items-center gap-2 text-[13px] text-foreground"><I size={15} className="text-primary-glow" />{String(t)}</li>; })}
            </ul>
          )}
