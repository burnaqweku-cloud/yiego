...
const errorLabels: Record<string, string> = {
  invalid_api_key: "The stored Claude API key is invalid or was revoked.",
  missing_api_key: "The Claude API key has not been configured.",
  provider_limit: "The Claude account has reached a usage or capacity limit.",
  provider_billing: "The Claude API account requires billing or credits.",
  model_unavailable: "The configured Claude model is unavailable for this API account.",
  provider_permission: "The Claude API account does not have permission for this request.",
};
...
export default function AdminAISupport() {
  ...
  const checkConnection = useCallback(async () => {
    ...
    if (error || data?.error || data?.status !== "ready") {
      ...
      setErrorMessage(errorLabels[data?.code ?? ""] ?? data?.error ?? error?.message ?? "Claude could not be reached.");
      return;
    }
    setConnection("ready"); setModel(data.model ?? "Claude");
  }, []);
  ...
  return <div className="space-y-7">
    <AdminPageHeader eyebrow="AI support" title="AI support control" description="Check the Claude connection and test exactly what customers receive from YieGo's 24/7 assistant." />
    ...
    <Card><CardContent><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="flex gap-3"><span className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl ${connection === "ready" ? "bg-success/[0.1] text-success" : "bg-amber/[0.1] text-amber"}`}>{connection === "ready" ? <CheckCircle2 size={20} /> : <TriangleAlert size={20} />}</span><div><h2 className="font-display text-lg font-semibold text-white">Claude connection</h2><p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">{connection === "checking" ? "Testing the secure server connection." : connection === "ready" ? `Connected securely${model ? ` using ${model}` : ""}. The API key stays on the server.` : errorMessage || "Claude is unavailable."}</p></div></div><Button variant="ghost" size="sm" onClick={() => void checkConnection()} disabled={connection === "checking"}>{connection === "checking" ? <Loader2 className="animate-spin" /> : <RefreshCw />}Test connection</Button></div></CardContent></Card>
