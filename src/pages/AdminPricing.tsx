...
import { AlertTriangle, BadgeDollarSign, Loader2, Search, Save, Tags, type LucideIcon } from "lucide-react";
...
export default function AdminPricing() {
  ...
  const visible = useMemo(() => {
    ...
    return rows.filter((row) => {
      const matchesNetwork = network === "all" || row.networks?.name === network;
      const matchesSearch = !needle
        || row.name.toLowerCase().includes(needle)
        || row.app_product_code?.toLowerCase().includes(needle)
        || row.networks?.name.toLowerCase().includes(needle);
  ...
  const save = async () => {
    ...
    setSaving(true);
    const { data, error } = await supabase.functions.invoke<{ status?: string; error?: string }>("admin-catalog-action", {
      body: {
    ...
    if (error || data?.error) {
      toast.error(data?.error ?? error?.message ?? "Could not update this bundle.");
      return;
    }
  ...
  return (
    <div className="space-y-7">
      <AdminPageHeader
        ...
        title="Data pricing"
        description="Set YieGo's customer prices while keeping live supplier costs visible for margin decisions."
      />

      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {([
          ["Bundles", rows.length, Tags],
          ["Custom prices", customPriced, BadgeDollarSign],
          ["Selling at cost", atCost, BadgeDollarSign],
          ["Below cost", belowCost, AlertTriangle],
        ] satisfies Array<[string, number, LucideIcon]>).map(([label, value, Icon]) => (
          <Card key={String(label)}><CardContent><Icon className="text-primary-glow" size={19} /><p className="mt-4 text-xs uppercase tracking-[0.14em] text-faint-foreground">{label as string}</p><p className="mt-2 font-display text-2xl font-semibold text-white">{loading ? "—" : value as number}</p></CardContent></Card>
        ))}
      </section>
        ...
        <CardContent>
          <div className="grid gap-3 md:grid-cols-[minmax(220px,1fr)_220px]">
            <label className="flex items-center gap-3 rounded-xl border border-white/10 bg-black/10 px-4 py-3">
              ...
              <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search bundle or product code" className="min-w-0 flex-1 bg-transparent text-sm text-foreground outline-none placeholder:text-faint-foreground" />
            </label>
            <select value={network} onChange={(event) => setNetwork(event.target.value)} className="onyx-field">
          ...
          <div className="mt-5 space-y-3 md:hidden">
            {visible.map((row) => (
              <article key={row.id} className="flex items-center gap-3 rounded-2xl border border-white/[0.08] bg-white/[0.025] p-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2"><p className="truncate font-semibold text-white">{row.name}</p><Badge variant={row.is_active ? "success" : "neutral"}>{row.is_active ? "Active" : "Hidden"}</Badge></div>
                  <p className="mt-1 text-xs text-muted-foreground">{row.networks?.name ?? "Network"} · Cost {formatGHS(supplierCost(row))}</p>
                  <p className="mt-2 font-display font-semibold text-primary-glow">Sell {formatGHS(Number(row.customer_price))}</p>
                </div>
                <AdminDetailsButton label={`View ${row.name} pricing`} onClick={() => openDetails(row)} />
              </article>
            ))}
          </div>
          ...
          <div className="mt-5 hidden overflow-x-auto md:block">
            <table className="w-full min-w-[820px] text-left text-sm">
              ...
              <tbody>{visible.map((row) => {
                const cost = supplierCost(row);
                const profit = Number(row.customer_price) - cost;
      ...
      <AdminRecordModal
        ...
        title={selected?.name ?? "Bundle"}
        subtitle={`${selected?.networks?.name ?? "Network"} · ${selected?.app_product_code ?? "Product"}`}
        fields={selected ? [
      ...
      >
        <div>
          <h3 className="font-display text-lg font-semibold text-white">Edit selling price</h3>
