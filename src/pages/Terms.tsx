...
const Terms = () => {
  return (
    <Layout>
      <SEOHead
        title="Terms of Service | DataSika"
        description="Read DataSika's terms of service. Learn about our data bundle delivery policies, payment terms, agent program rules, and refund process for MTN, Telecel & AirtelTigo in Ghana."
        path="/terms"
      />
      <div className="container py-8 md:py-14 max-w-2xl">
        ...
        <div className="bg-card border border-border rounded-2xl p-6 md:p-8 card-shadow mb-8">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-display font-bold">DataSika Terms of Service</h1>
          ...
          <p className="mt-5 text-sm text-muted-foreground leading-relaxed">
            Welcome to DataSika ("DataSika", "we", "us", "our"). These Terms of Service ("Terms") govern your access to and use of the DataSika website, platform, services, and any related features, including our data bundle purchase services and agent reseller system (collectively, the "Service").
          </p>
          <p className="mt-3 text-sm text-muted-foreground leading-relaxed">
            By accessing or using DataSika, you confirm that you have read, understood, and agreed to be bound by these Terms. If you do not agree with these Terms, you must not use the Service.
          </p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <Section number="1" title="Eligibility and Account Registration">
            <ol className="list-decimal pl-4 space-y-2 text-sm text-muted-foreground marker:text-muted-foreground">
              <li>You must be at least 18 years old, or have permission from a parent or legal guardian, to use DataSika.</li>
              <li>You may be required to create an account to access certain features.</li>
              <li>You agree to provide accurate and complete information during registration.</li>
              <li>You are responsible for maintaining the confidentiality of your login credentials.</li>
              <li>You are responsible for all activity that occurs under your account.</li>
            </ol>
            <p className="text-sm text-muted-foreground mt-3">DataSika is not responsible for unauthorized access resulting from your failure to secure your account.</p>
          </Section>

          <Section number="2" title="Services Provided">
            <p className="text-sm text-muted-foreground">DataSika provides a digital platform that allows users to purchase and deliver mobile data bundles for supported Ghanaian telecom networks, including but not limited to:</p>
            <ul className="list-disc pl-4 mt-2 space-y-1 text-sm text-muted-foreground marker:text-muted-foreground">
              <li>MTN Ghana</li>
              <li>Telecel Ghana</li>
              <li>AirtelTigo</li>
            </ul>
            <p className="text-sm text-muted-foreground mt-3">DataSika acts as a reseller and delivery facilitator. Data delivery is dependent on network availability, telecom provider processing, and system integration.</p>
          ...
          <Section number="4" title="Airtime Debt and Unsupported Numbers">
            ...
            <p className="text-sm text-muted-foreground mt-2">If the recipient number owes airtime or has restrictions, delivery may delay or fail. DataSika is not responsible for failures caused by the recipient's telecom status. Additionally, some SIM types (including certain TurboNet SIM cards) may not be supported for bundle delivery.</p>
          </Section>

          <Section number="5" title="Payments">
            <SubSection title="5.1 Payment Methods">
              <p className="text-sm text-muted-foreground">DataSika accepts payments through supported channels such as Mobile Money (MoMo) and Paystack or other supported payment processors.</p>
            </SubSection>
            ...
            <SubSection title="5.3 Fees and Charges">
              <p className="text-sm text-muted-foreground">Payment processors may apply transaction charges, service fees, or taxes. These charges are determined by the payment provider and may not be controlled by DataSika. By using DataSika, you acknowledge that transaction charges may apply depending on your payment method.</p>
            </SubSection>
          </Section>

          <Section number="6" title="Wallet Funding and Account Usage">
            <p className="text-sm text-muted-foreground">DataSika may provide users with wallet-related features for convenience. Wallet balances can be used for purchases, subject to platform policies. Wallet deposits and withdrawals may be subject to verification and processing time. You are advised to always make purchases while logged into your DataSika account for easier tracking and support resolution.</p>
          </Section>

          <Section number="7" title="Pricing and Availability">
            <p className="text-sm text-muted-foreground">All prices displayed on DataSika are in Ghana Cedis (GHS) unless stated otherwise. Prices may change at any time due to supplier price changes, network changes, or operational adjustments. DataSika reserves the right to update pricing without prior notice.</p>
          </Section>

          <Section number="8" title="Wrong Number Policy">
            <p className="text-sm text-muted-foreground">You are solely responsible for entering the correct recipient phone number before confirming your purchase. Once a data bundle is delivered to a phone number, the transaction cannot be reversed or recovered. DataSika will not be responsible for losses caused by wrong phone number entry, incorrect network selection, or user mistakes during checkout.</p>
          </Section>
          ...
          <Section number="10" title="Agents and Reseller Program">
            <p className="text-sm text-muted-foreground">DataSika offers an Agent Program that allows approved users ("Agents") to operate a personal DataSika store link and set their own selling prices above the base agent prices provided by DataSika.</p>
            <SubSection title="10.1 Agent Approval">
              <p className="text-sm text-muted-foreground">Agent applications are subject to review and approval. DataSika reserves the right to approve or reject any application at its discretion.</p>
            </SubSection>
            <SubSection title="10.2 Agent Pricing">
              <p className="text-sm text-muted-foreground">Agents may set their own prices, and their profits are based on the difference between the base agent price and the agent's selling price. DataSika is not responsible for prices set by agents, agent profit margins, or agent business decisions.</p>
            </SubSection>
            <SubSection title="10.3 Customer-Agent Relationship">
              <p className="text-sm text-muted-foreground">Customers who purchase through an agent's store link are purchasing through that agent's store system. Agents are expected to provide basic customer assistance and communication. However, DataSika remains responsible for the platform's technical delivery process.</p>
            </SubSection>
            <SubSection title="10.4 Misuse and Termination">
              <p className="text-sm text-muted-foreground">DataSika may suspend or terminate any agent account for fraudulent activity, abuse of the platform, misrepresentation, violations of these Terms, or harmful or unethical business practices.</p>
            </SubSection>
          </Section>

          <Section number="11" title="Prohibited Activities">
            <p className="text-sm text-muted-foreground">You agree not to use DataSika for any unlawful, abusive, or harmful purpose. Prohibited activities include but are not limited to:</p>
            <ul className="list-disc pl-4 mt-2 space-y-1 text-sm text-muted-foreground marker:text-muted-foreground">
              <li>Attempting to reverse-engineer or exploit the platform</li>
              <li>Using DataSika to commit fraud</li>
          ...
          <Section number="12" title="System Availability and Maintenance">
            <p className="text-sm text-muted-foreground">DataSika may occasionally perform system upgrades or maintenance that could temporarily interrupt service availability. We do not guarantee uninterrupted access to the Service.</p>
          </Section>

          <Section number="13" title="Limitation of Liability">
            <p className="text-sm text-muted-foreground">To the maximum extent permitted by law, DataSika shall not be liable for any loss, damages, or claims arising from: telecom provider delays or failures, network congestion or downtime, payment processor delays, wrong phone number submissions, airtime debt restrictions, unsupported SIM card issues, temporary system outages, or delays caused by high order volume.</p>
            <p className="text-sm text-muted-foreground mt-2">DataSika's liability, if any, shall not exceed the total amount paid by the user for the specific transaction under dispute.</p>
          </Section>

          <Section number="14" title="Indemnification">
            <p className="text-sm text-muted-foreground">You agree to indemnify and hold harmless DataSika, its owners, staff, and partners from any claims, liabilities, damages, losses, or expenses arising from your misuse of the Service, your violation of these Terms, or your fraudulent or unlawful actions.</p>
          </Section>

          <Section number="15" title="Privacy Policy">
            <p className="text-sm text-muted-foreground">Your use of DataSika is also governed by our <Link to="/privacy" className="text-primary hover:underline font-medium">Privacy Policy</Link>. By using DataSika, you consent to the collection and processing of your data as described in our Privacy Policy.</p>
          </Section>

          <Section number="16" title="Changes to These Terms">
            <p className="text-sm text-muted-foreground">DataSika may update or modify these Terms at any time. Updates will become effective immediately upon posting on our website. Continued use of the Service after updates means you accept the revised Terms.</p>
          </Section>

          <Section number="17" title="Termination of Service">
            <p className="text-sm text-muted-foreground">DataSika may suspend or terminate your access to the Service at any time, without notice, if you violate these Terms, engage in fraud or suspicious transactions, abuse the platform, or harm the platform or other users. Termination may result in loss of access to your account.</p>
          </Section>
          ...
          <Section number="19" title="Contact Information">
            <p className="text-sm text-muted-foreground">For support or inquiries, you may contact DataSika through:</p>
            <ul className="list-disc pl-4 mt-2 space-y-1 text-sm text-muted-foreground marker:text-muted-foreground">
