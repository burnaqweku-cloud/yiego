-- Fix GROUP BY error in generate_bulk_dispatch_batches
-- The previous version had a leftover broken aggregation block that selected
-- _eligible.bundle_size_gb without grouping. Replace the function with a
-- cleaned-up version. No business/order/payment logic changes.
...
BEGIN
  ...
  -- Pull queued orders for this network from both tables, FIFO
  ...
  WHILE v_offset < v_total_count LOOP
    ...
    -- Insert batch items for the current FIFO window
    WITH chunk AS (
    ...
    -- Build bundle-size summary { "1": 12, "2": 3, ... } for audit
