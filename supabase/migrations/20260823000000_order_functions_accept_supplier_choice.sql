-- Let the customer's chosen supplier reach the order.
--
-- The two order-creating functions ranked suppliers themselves and gave the
-- caller no say. Now they take an optional supplier: when the customer picked
-- one in the shop it is honoured exactly, and when they did not the ranking
-- still applies. The old three-argument form is dropped rather than left as an
-- overload, so a three-argument call cannot become ambiguous.


drop function if exists phase1.create_wallet_data_order(uuid, text, text);

CREATE OR REPLACE FUNCTION phase1.create_wallet_data_order(p_user_id uuid, p_product_code text, p_recipient_phone text, p_supplier_id uuid DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
drop function if exists phase1.prepare_data_order(uuid, text, text);

CREATE OR REPLACE FUNCTION phase1.prepare_data_order(p_user_id uuid, p_product_code text, p_recipient_phone text, p_supplier_id uuid DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
  ...
  select m.* into v_mapping from phase1.supplier_product_mappings m join phase1.suppliers s on s.id = m.supplier_id where m.product_id = v_product.id and m.is_active and s.status = 'active' and (p_supplier_id is null or m.supplier_id = p_supplier_id) order by s.display_order asc, m.id asc limit 1;
...
-- Recreating the order functions reset their privileges to the Postgres
-- default, which is EXECUTE for PUBLIC — briefly letting anon and authenticated
-- callers invoke them straight through PostgREST. Restore the original posture:
-- only the service role, reached through an edge function that authenticates
-- the caller first.
revoke all on function phase1.create_wallet_data_order(uuid, text, text, uuid) from public, anon, authenticated;
revoke all on function phase1.prepare_data_order(uuid, text, text, uuid) from public, anon, authenticated;
grant execute on function phase1.create_wallet_data_order(uuid, text, text, uuid) to service_role;
grant execute on function phase1.prepare_data_order(uuid, text, text, uuid) to service_role;
notify pgrst, 'reload schema';
