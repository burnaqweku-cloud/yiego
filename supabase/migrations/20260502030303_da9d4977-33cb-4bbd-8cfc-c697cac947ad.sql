...
AS $function$
  ...
  SELECT
    ...
    c.tg_uid AS chat_id,
    COALESCE(NULLIF(BTRIM(tl.first_name), ''), NULLIF(BTRIM(tku.first_name), '')) AS first_name,
    tl.username,
    c.earned
  FROM collapsed c
  LEFT JOIN public.telegram_links tl ON tl.chat_id = c.tg_uid
  LEFT JOIN public.telegram_known_users tku ON tku.telegram_user_id = c.tg_uid
...
SET first_name = NULLIF(BTRIM(tku.first_name), '')
FROM public.telegram_known_users tku
WHERE tl.chat_id = tku.telegram_user_id
  AND NULLIF(BTRIM(tku.first_name), '') IS NOT NULL
  AND (tl.first_name IS NULL OR BTRIM(tl.first_name) = '');