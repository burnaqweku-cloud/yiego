...
-- Create SECURITY DEFINER RPC for agent withdrawal requests
-- This replaces direct client inserts into agent_wallet_transactions (blocked by RLS)
...
BEGIN
  ...
  -- 5. Validate MoMo number (Ghana format: 10 digits starting with 0)
  ...
  -- 7. Check for existing pending withdrawal (prevent spam)
  IF EXISTS (
  ...
  -- 8. Get available balance from agent_wallets
  ...
  -- 9a. Insert withdrawal request
  INSERT INTO public.agent_withdrawals (agent_id, amount_ghs, momo_number, momo_network, status)
  VALUES (v_agent_id, p_amount, v_cleaned_phone, p_momo_network, 'pending')
