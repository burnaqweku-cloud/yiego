...
BEGIN
  ...
  WHILE v_offset < v_total_count LOOP
    v_chunk_size := LEAST(50, v_total_count - v_offset);
    ...
    WITH chunk AS (
      ...
      FROM _eligible
      WHERE seq > v_offset AND seq <= v_offset + v_chunk_size
      ORDER BY seq
    )
    INSERT INTO public.dispatch_batch_items (
      batch_id, order_table, order_id, order_uuid, recipient_number, network, bundle_size_gb
    )
    SELECT v_batch_id, order_table, order_id, order_uuid, recipient, network, bundle_size_gb
    FROM chunk;

    UPDATE public.orders o
    SET queue_state = 'batched', batch_id = v_batch_id, batched_at = now()
    WHERE o.id IN (
      SELECT order_uuid FROM _eligible
      WHERE order_table = 'orders' AND seq > v_offset AND seq <= v_offset + v_chunk_size
    );
    ...
    WHERE a.id IN (
      SELECT order_uuid FROM _eligible
      WHERE order_table = 'agent_orders' AND seq > v_offset AND seq <= v_offset + v_chunk_size
    );
