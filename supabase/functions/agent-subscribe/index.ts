...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const init = await initializePaystackTransaction({ email: auth.user.email, amount, reference, currency: "GHS", callbackUrl: `${site}/agent?paid=1`, metadata: { purpose: "agent_subscription", agentId: agent.id, promoId: quote?.promo?.id ?? null } });
    if (!init.ok || !init.payload?.status) return jsonResponse({ error: init.payload?.message ?? "Could not start payment" }, { status: 502 });
    const { error } = await supabase.from("payment_intents").insert({ provider: "paystack", purpose: "agent_subscription", status: "pending", user_id: auth.user.id, amount, currency: "GHS", provider_reference: reference, authorization_url: init.payload.data.authorization_url, metadata: { accessCode: init.payload.data.access_code, agent_id: agent.id, promo_id: quote?.promo?.id ?? null, monthly: quote?.monthly, percent_off: quote?.promo?.percent_off ?? 0 } });
    if (error) return jsonResponse({ error: error.message }, { status: 500 });
    return jsonResponse({ status: "success", data: { authorizationUrl: init.payload.data.authorization_url, amount, reference } });
