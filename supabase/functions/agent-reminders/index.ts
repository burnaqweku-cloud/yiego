...
/* Daily at 08:00: email agents whose month ends in 3 days, and on the day. */
  ...
  try {
    ...
    for (const a of (due ?? []) as Array<{ agent_id: string; user_id: string; store_name: string; paid_until: string; kind: string }>) {
      ...
      const html = a.kind === "3d"
        ...
        : wrap("Your agent month ends today", `<p>Hi ${profile.full_name ?? ""},</p><p>Today is the last day of your DataYego agent plan for <b>${a.store_name}</b>. If it isn't renewed by tonight your store will pause — nothing is lost, it reopens the moment you pay.</p><p>Fee: <b>GH₵ ${price}</b>.</p>`);
      const r = await sendEmail({ to: profile.email, subject: a.kind === "3d" ? "Your DataYego agent plan ends in 3 days" : "Your DataYego agent plan ends today", replyTo: "support@yiego.shop", html });
